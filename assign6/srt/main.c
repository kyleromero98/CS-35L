...
// Global scene for threads to use
scene_t scene;

// Global number of threads
int nthreads;

// Where we will store the values for printing
float storedValues[width][height][3];

// Function to be threaded
void * threadedImageSplitter(void* thread_num) {
    Vec3 camera_pos;
    ...
    /* for every pixel */
    // we just had to alter this line to account for multiple threads
    // computing these values in parallel
    for( int px= *(int*)thread_num; px<width; px += nthreads)
    {
	    ...
	    storedValues[px][py][0] = scaled_color[0];
	    storedValues[px][py][1] = scaled_color[1];
	    storedValues[px][py][2] = scaled_color[2];
        }
    }
    return NULL;
}
...
main( int argc, char **argv )
{
    ...
    // Arrays for threads and thread ids
    pthread_t threads[nthreads];
    int thread_ids[nthreads];

    // Create each thread and error check
    for (int i = 0; i < nthreads; i++) {
      thread_ids[i] = i;
      int error = pthread_create(&threads[i], NULL, threadedImageSplitter, &thread_ids[i]);
      if (error) {
	fprintf(stderr, "Error %d: Creating threads failed\n", error);
	exit(1);
      }
    }
    ...
    for (int i = 0; i < nthreads; i++) {
      ...
      if (error) {
	fprintf(stderr, "Error %d: Joining threads failed\n", error);
	exit(1);
      }
    }
	...
	printf( "%.0f %.0f %.0f\n",
		storedValues[i][j][0], storedValues[i][j][1], storedValues[i][j][2]);
      }
      printf("\n");
    }
